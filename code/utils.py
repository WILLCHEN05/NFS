...
def evaluate(X, y, args):
	...
	if args.task == 'regression':
		...
		if args.evaluate == 'mae':
			r_mae = cross_val_score(model, X, y, cv=fold_used, 
				scoring='neg_mean_absolute_error').mean()
			return r_mae
		elif args.evaluate == 'mse':
			r_mse = cross_val_score(model, X, y, cv=fold_used, 
				scoring='neg_mean_squared_error').mean()
			return r_mse
		elif args.evaluate == 'r2':
			r_r2 = cross_val_score(model, X, y, cv=fold_used).mean()
			return r_r2
		...
		if args.evaluate == 'f_score':
			...
			s = cross_val_score(model, X, y, scoring='f1_micro', cv=fold_used).mean() # cv = 10
...
class EvaluationService(Service):
	def exposed_evaluate(self, X, d, task, i_model, i_evl):
		...
		if task == 'regression':
			...
			# data[:, -1] = data[:, -1].astype(str) #Xudong
			if i_model == 'RF':
