...
class Controller:
	...
	def _create_variable(self):
		# self.input0 = tf.Variable(
		# 	tf.random_uniform(shape=[self.num_feature,self.num_op],
		# 		minval=0,maxval=1), name='input0', dtype=tf.float32, trainable=False)
	...
	def _create_loss(self):
		...
		for batch_count in range(self.num_batch):
			...
			# loss_batch = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
			# 	labels=action, logits=self.concat_output)) * self.rewards[batch_count]
			
			# loss_batch = - tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
			# 	labels=action, logits=self.concat_output) * reward)
...
class Controller_pure:
	...
	def _create_loss(self):
		...
		for batch_count in range(self.num_batch):
			...
			action_index = tf.stack([list(range(self.num_action)), action], axis=1)
			action_probs = tf.squeeze(tf.nn.softmax(self.concat_output))
			# action_probs = action_probs + 1e-10     #Xudong
			pick_action_prob = tf.gather_nd(action_probs, action_index)
			# pick_action_prob = pick_action_prob + 1e-10     #Xudong
			# loss_batch = tf.reduce_sum(-tf.log(pick_action_prob + 1e-10) * reward) #Xudong 
			# loss_entropy = tf.reduce_sum(-action_probs * tf.log(action_probs + 1e-10)) * self.reg #Xudong
			# print("reward: ", reward)
			...
			#print("loss_batch: " + str(loss_batch))
			#print("loss_entropy: " + str(loss_entropy))

			self.loss += loss_batch + loss_entropy
			#print("before batch: " + str(self.loss))
		self.loss /= self.num_batch
		#print("num batch: " + str(self.num_batch))
		#print("after batch: " + str(self.loss))
