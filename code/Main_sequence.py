...
def parse_args():
	...
	parser.add_argument('--fold',nargs='?',
		default=5, help='fold used in cross validation in sklearn')
		...
		for action in feature_actions:
			if action == 0:
				...
				if action_unary == 0:
					...
					while (np.any(copy == 0)):
						copy = copy + 1e-5
					copy = np.squeeze(np.log(abs(np.array(copy)))) ### Xudong 
				elif action_unary == 3:
					while (np.any(copy == 0)):
						copy = copy + 1e-5
					copy = np.squeeze(1 / (np.array(copy)))   ###Xudong 
				...
				target = np.array(X[target_feature_name].values) # Xudong
				...
				# 	copy = np.squeeze(copy / (X[target_feature_name].values+1e-3)) ###Xudong 
				...
				if action_binary == 0:
					...
					while (np.any(target == 0)):
						target = target + 1e-5
					copy = np.squeeze(copy / target) ###Xudong 
				elif action_binary == 4:
					copy = np.squeeze(mod_column(copy, X[target_feature_name].values))
...
def random_run(num_random_sample, model, l=None, p=None):
	...
	if args.multiprocessing:	
		if args.package == 'weka':
			pool = Pool(num_process, initializer=init, initargs=(l, p)) # num_proess // 2 Xudong
		elif args.package == 'sklearn':
			pool = Pool(num_process)    # num_proess // 2 Xudong
...
def train(model, l=None, p=None):
	...
	with tf.Session(config=config) as sess:
		...
		model_result = -10000.0 # Xudong model_result = 0
		train_set, values = [], []
			...
			if args.RL_model == 'AC':
				...
				for i in range(model.num_action):
					...
					for j in range(base.shape[1]):
						...
						base_order = base_order * np.power(args.lambd, j) #Xudong
						# rewards_order += base_order
						rewards_order = rewards_order.astype(float)  #Xudong
						rewards_order += base_order.astype(float)  #Xudong
